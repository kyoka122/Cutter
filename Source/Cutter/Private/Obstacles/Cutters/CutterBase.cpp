...
void ACutterBase::RegisterStageShapeData(const TScriptInterface<IStageShape>& stageShape)
{
	_stageShape = stageShape;
}

void ACutterBase::RegisterScoreAddFunc(const ScoreAddFunc& func)
{
	_scoreAddFunc = func;
}

void ACutterBase::RegisterReleaseFunc(const TFunction<void(ACutterBase* cutter)>& releaseFunc)
{
	_releaseFunc = releaseFunc;
}

void ACutterBase::RegisterStaticMeshEvent(UStaticMeshComponent* staticMeshComponent, const OverlapFunc& func)
{
